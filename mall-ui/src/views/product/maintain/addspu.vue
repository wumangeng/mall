      ...
      <el-col :span="24" v-show="step==0">
        <el-card class="box-card" style="width:80%;margin:20px auto">
          <el-form ref="spuBaseForm" :model="spu" label-width="120px" :rules="spuBaseInfoRules">
            ...
            <el-form-item style="text-align: -webkit-center;">
...
<script>
  ...
  import {fetchMemberLevelList,getSaleAttrList,getAttrGroupWithAttrs,addSpu} from '@/api/product/addspu'
  ...
  export default {
    ...
    methods: {
      ...
      getMemberLevels() {
       ...
       fetchMemberLevelList().then(({data}) => {
      ...
      getShowSaleAttr() {
        ...
        if (!this.dataResp.steped[1]) {
          getSaleAttrList({ page: 1, limit: 1000 },this.spu.catelogId).then(({data}) => {
            console.log("获取当前分类可以使用的销售属性-->",data.data)
      ...
      showBaseAttrs() {
        if (!this.dataResp.steped[0]) {
          getAttrGroupWithAttrs(this.spu.catelogId).then(({data}) => {
            //先对表单的baseAttrs进行初始化
